...
export default function GenesisShipDashboard() {
    const baseUrl = (process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8001").replace(/\/$/, "");
    ...
    // Defensive: neutralize full-screen overlays injected by extensions/devtools
    useEffect(() => {
        const disableOverlays = () => {
            ...
            for (const el of nodes) {
                if (el.dataset.peekoriOverlayBlocker === "1") continue;
                const style = window.getComputedStyle(el);
                if (style.position !== "fixed") continue;
                const z = parseInt(style.zIndex || "0", 10);
                if (Number.isNaN(z) || z < 999) continue;
                const rect = el.getBoundingClientRect();
                if (rect.left > 0 || rect.top > 0) continue;
                if (rect.width < vw || rect.height < vh) continue;

                el.dataset.peekoriOverlayBlocker = "1";
                el.style.pointerEvents = "none";
            }

            // Common dark-reader/overlay class names
            const known = document.querySelectorAll<HTMLElement>(
                ".darkreader, .darkreader--root, .darkreader--inline, .darkreader--overlay"
            );
            known.forEach((el) => {
                el.dataset.peekoriOverlayBlocker = "1";
                el.style.pointerEvents = "none";
        ...
        disableOverlays();
        const id = window.setInterval(disableOverlays, 1000);
        ...
        return (
            ...
            >
                <motion.div
                    ...
                    className="relative z-[9999] w-full max-w-md pointer-events-auto"
                >
                    <form
                ...
                <div>
                    <div className="flex items-center justify-between gap-4">
                        <motion.h2
                            ...
                            transition={{ delay: 0.1 }}
                        >
                            {printerKey === "genesis" ? "Genesis" : "Yara"} x Peekori
                            Dashboard
                        </motion.h2>
